...
COMPOSE_PROFILE=${COMPOSE_PROFILE:="raspios"}
...
# build vars
ASSETS_DIRECTORY=./assets/$COMPOSE_PROFILE
...
CMDLINE_CONTENTS="selinux=0 dwc_otg.lpm_enable=0 console=tty1 rootwait rw nfsroot=192.168.0.66:/mnt/nfsshare/$IMG_FILENAME,v3 ip=dhcp root=/dev/nfs elevator=deadline systemd.log_level=info systemd.log_target=console systemd.debug-shell=1 init=/boot/apply-config.sh"
...
if [ -f "$FSTAB_PATH" ]; then
    ...
    FSTAB_CONTENTS="proc                       /proc           proc   defaults          0       0\n192.168.0.66:/mnt/nfsshare/$IMG_FILENAME/boot/firmware  /boot/firmware  nfs    defaults          0       2\n192.168.0.66:/mnt/nfsshare/$IMG_FILENAME                /               nfs    defaults,noatime  0       1"
