...
```
selinux=0 dwc_otg.lpm_enable=0 console=tty1 rootwait rw nfsroot=192.168.0.66:/mnt/nfsshare/2023-12-11-raspios-bookworm-arm64-lite,v3 ip=dhcp root=/dev/nfs elevator=deadline systemd.log_level=info systemd.log_target=console systemd.debug-shell=1
```
...
192.168.0.66:/mnt/nfsshare/2023-12-11-raspios-bookworm-arm64-lite/boot/firmware /boot/firmware nfs defaults 0 2
192.168.0.66:/mnt/nfsshare/2023-12-11-raspios-bookworm-arm64-lite / nfs defaults,noatime 0 1
```
...
#### Implementation
The shell script (named [`apply-config.sh`](https://github.com/andrewiankidd/project-iluvatar/blob/master/src/bootstrap/netboot/assets/raspios/boot/apply-config.sh) here), is a highly modified version of the `firstrun.sh` file created by the Raspberry Pi Imager and is added to the OS partitions boot directory alongside a `.config` file, which is used to provide configuration values to the script - [An example file is here](https://github.com/andrewiankidd/project-iluvatar/blob/master/src/bootstrap/netboot/assets/raspios/boot/.config.example)
...
```
selinux=0 dwc_otg.lpm_enable=0 console=tty1 rootwait rw nfsroot=192.168.0.66:/mnt/nfsshare/2023-12-11-raspios-bookworm-arm64-lite,v3 ip=dhcp root=/dev/nfs elevator=deadline systemd.log_level=info systemd.log_target=console systemd.debug-shell=1 init=/boot/apply-config.sh
```
...
Within the project-iluvatar repo, I've created a ['debian-builder' Dockerfile](https://github.com/andrewiankidd/project-iluvatar/blob/master/src/bootstrap/netboot/debian-builder.Dockerfile), as well as a Bash script called [build-debian.sh](https://github.com/andrewiankidd/project-iluvatar/blob/master/src/bootstrap/netboot/scripts/build-debian.sh) that tries to do all this automatically.

When I run `docker compose --profile raspios up` the debian-builder is fired up. It will then download the image, extract it, mount and copy the files, patch `cmdline.txt` in the boot files and finally `/etc/fstab` in the OS files

![Screenshot of debian-builder image running](../../../../assets/docs/guides/bootstrap/prep/debian-builder.png)
